...
func runPrestateBlock(t *testing.T, prestatePath string, hooks *tracing.Hooks) {
	...
	if err := rlp.DecodeBytes(common.FromHex(prestate.Input), tx); err != nil {
	...
	}, nil, trie.NewStackTrie(nil), types.DefaultBlockConfig)
	...
	chainConfig := prestate.Genesis.Config
	blockContext := core.NewEVMBlockContext(block.Header(), nil, new(common.Address), chainConfig, testState.StateDB)

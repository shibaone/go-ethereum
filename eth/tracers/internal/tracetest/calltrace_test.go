...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			evm := vm.NewEVM(blockContext, txContext, statedb, test.Genesis.Config, vm.Config{Tracer: tracer}, firehose.NoOpContext)
...
func TestInternals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		evm := vm.NewEVM(blockContext, txContext, statedb, params.MainnetChainConfig, vm.Config{Tracer: tc.tracer}, firehose.NoOpContext)
		msg := &core.Message{
			To:                &to,
			From:              origin,
			Value:             big.NewInt(0),

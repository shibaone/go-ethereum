...
func (p *Printer) CaptureArbitrumTransfer(env *vm.EVM, from, to *common.Address, value *big.Int, before bool, purpose string) {
	fmt.Printf("CaptureArbitrumTransfer: env=%v, from=%v, to=%v, value = %v, before = %v, purpose = %s\n", env, from, to, value, before, purpose)
}

func (p *Printer) CaptureArbitrumStorageGet(key common.Hash, depth int, before bool) {
	fmt.Printf("CaptureArbitrumStorageGet: key=%v, depth=%d, before=%v\n", key, depth, before)
}

func (p *Printer) CaptureArbitrumStorageSet(key, value common.Hash, depth int, before bool) {
	fmt.Printf("CaptureArbitrumStorageGet: key=%v, value=%v, depth=%d, before=%v\n", key, value, depth, before)
}

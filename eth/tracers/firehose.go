...
func (f *Firehose) OnBlockStart(event tracing.BlockEvent) {
	...
	f.block = &pbeth.Block{
		...
		Header: newBlockHeaderFromChainHeader(hash, header, firehoseBigIntFromNative(new(big.Int).Add(event.TD, block.Difficulty())), *f.applyBackwardCompatibility),
		Ver:    4,
...
func newBlockHeaderFromChainHeader(hash common.Hash, h *types.Header, td *pbeth.BigInt, applyBackwardCompatibility bool) *pbeth.BlockHeader {

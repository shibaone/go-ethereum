...
func (f *Firehose) CaptureTxStart(evm *vm.EVM, tx *types.Transaction) {
	...
	signer := types.MakeSigner(evm.ChainConfig(), evm.Context.BlockNumber)
...
func validateFirehoseKnownTransactionType(txType byte, isKnownFirehoseTxType bool) error {
	...
	if err != nil {
		if err == types.ErrTxTypeNotSupported {
			if isKnownFirehoseTxType {
				return fmt.Errorf("firehose known type %d but encoding RLP of receipt led to 'types.ErrTxTypeNotSupported'", txType)
			}
	...
	if err != nil {
		if err == types.ErrTxTypeNotSupported {
			if isKnownFirehoseTxType {
				return fmt.Errorf("firehose known type %d but decoding of RLP of receipt led to 'types.ErrTxTypeNotSupported'", txType)
			}

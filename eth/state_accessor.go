...
func (eth *Ethereum) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64, firehoseContext *firehose.Context) (*core.Message, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	...
	if !eth.blockchain.Config().IsFeynman(block.Number(), block.Time()) {
		systemcontracts.UpgradeBuildInSystemContract(eth.blockchain.Config(), block.Number(), parent.Time(), block.Time(), statedb, firehoseContext)
	}
			...
			if posa, ok := eth.Engine().(consensus.PoSA); ok {
				if isSystem, _ := posa.IsSystemTransaction(tx, block.Header()); isSystem {
					...
					if balance.Cmp(common.U2560) > 0 {
						statedb.SetBalance(consensus.SystemAddress, uint256.NewInt(0), firehoseContext, firehose.BalanceChangeReason("reward_transfaction_fee"))
						statedb.AddBalance(block.Header().Coinbase, balance, false, firehoseContext, firehose.BalanceChangeReason("reward_transfaction_fee"))
					}

	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				vmenv := vm.NewEVM(blockContext, txContext, statedb, config, vm.Config{NoBaseFee: true}, firehose.NoOpContext)
			...
			txContext := core.NewEVMTxContext(msg)
			vmenv := vm.NewEVM(blockContext, txContext, state, config, vm.Config{NoBaseFee: true}, firehose.NoOpContext)
			gp := new(core.GasPool).AddGas(math.MaxUint64)
			// nolint : contextcheck
			result, _ := core.ApplyMessage(vmenv, msg, gp, context.Background())
			if state.Error() == nil {

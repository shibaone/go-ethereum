...
func (s *StateDB) ApplyMVWriteSet(writes []blockstm.WriteDescriptor) {
	for i := range writes {
		...
		if path.IsState() {
			...
			s.SetState(addr, stateKey, state, firehose.NoOpContext)
			...
			case BalancePath:
				s.SetBalance(addr, sr.GetBalance(addr), firehose.NoOpContext, firehose.IgnoredBalanceChangeReason)
			case NoncePath:
				s.SetNonce(addr, sr.GetNonce(addr), firehose.NoOpContext)
			case CodePath:
				s.SetCode(addr, sr.GetCode(addr), firehose.NoOpContext)
			case SuicidePath:

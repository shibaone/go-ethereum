...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		state.AddBalance(addr, uint256.NewInt(uint64(11*i)), 0x0)
...
func TestCopy(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		obj.AddBalance(uint256.NewInt(uint64(i)), 0x0)
	...
	for i := byte(0); i < 255; i++ {
		...
		origObj.AddBalance(uint256.NewInt(2*uint64(i)), 0x0)
		copyObj.AddBalance(uint256.NewInt(3*uint64(i)), 0x0)
		ccopyObj.AddBalance(uint256.NewInt(4*uint64(i)), 0x0)
...
func newTestAction(addr common.Address, r *rand.Rand) testAction {
	actions := []testAction{
		...
		},
		{
			name: "AddBalance",
			fn: func(a testAction, s *StateDB) {
				s.AddBalance(addr, uint256.NewInt(uint64(a.args[0])), 0x0)
			},
			args: make([]int64, 1),
		},
		{

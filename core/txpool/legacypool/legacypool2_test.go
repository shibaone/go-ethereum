...
func fillPool(t testing.TB, pool *LegacyPool) {
	...
	for i := 0; i < 384; i++ {
		...
		pool.currentState.AddBalance(crypto.PubkeyToAddress(key.PublicKey), big.NewInt(10000000000), false, firehose.NoOpContext, "test")
		// Add executable ones
...
func TestTransactionFutureAttack(t *testing.T) {
	...
	{
		key, _ := crypto.GenerateKey()
		pool.currentState.AddBalance(crypto.PubkeyToAddress(key.PublicKey), big.NewInt(100000000000), false, firehose.NoOpContext, "test")

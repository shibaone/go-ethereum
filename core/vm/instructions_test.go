...
func TestOpTstore(t *testing.T) {
	var (
		...
		contract       = NewContract(contractRef, AccountRef(to), new(uint256.Int), 0, firehose.NoOpContext)
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		var (
			env            = NewEVM(BlockContext{}, TxContext{BlobHashes: tt.hashes}, nil, params.TestChainConfig, Config{}, firehose.NoOpContext)
			stack          = newstack()
			pc             = uint64(0)
			evmInterpreter = env.interpreter
		)

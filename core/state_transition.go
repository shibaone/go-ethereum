...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.ChainConfig().Parlia != nil {
		st.state.AddBalance(consensus.SystemAddress, new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), effectiveTip), false, st.firehoseContext, firehose.BalanceChangeReason("reward_transaction_fee"))
	} else {

...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal, _ *[]*types.Receipt, _ *[]*types.Transaction, _ *uint64, firehoseContext *firehose.Context) error {
	// Finalize is different with Prepare, it can be used in both block verification.
	if !beacon.IsPoSHeader(header) {
		beacon.ethone.Finalize(chain, header, state, txs, uncles, nil, nil, nil, nil, firehoseContext)
		return nil
	}
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal, firehoseContext *firehose.Context) (*types.Block, []*types.Receipt, error) {
	...
	beacon.Finalize(chain, header, state, &txs, uncles, withdrawals, nil, nil, nil, firehoseContext)

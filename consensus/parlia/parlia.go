...
func (p *Parlia) distributeFinalityReward(chain consensus.ChainHeaderReader, state *state.StateDB, header *types.Header,
	...
	usedGas *uint64, mining bool, firehoseContext *firehose.Context) error {
	...
	return p.applyTransaction(msg, state, header, cx, txs, receipts, systemTxs, usedGas, mining, firehoseContext)
}
...
func (p *Parlia) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	uncles []*types.Header, _ []*types.Withdrawal, receipts *[]*types.Receipt, systemTxs *[]*types.Transaction, usedGas *uint64, firehoseContext *firehose.Context) error {
	...
	if p.chainConfig.IsFeynman(header.Number, header.Time) {
		systemcontracts.UpgradeBuildInSystemContract(p.chainConfig, header.Number, parent.Time, header.Time, state, firehoseContext)
	}

	if p.chainConfig.IsOnFeynman(header.Number, parent.Time, header.Time) {
		err := p.initializeFeynmanContract(state, header, cx, txs, receipts, systemTxs, usedGas, false, firehoseContext)
		if err != nil {
	...
	if p.chainConfig.IsPlato(header.Number) {
		if err := p.distributeFinalityReward(chain, state, header, cx, txs, receipts, systemTxs, usedGas, false, firehoseContext); err != nil {
			return err
		}
	}
	...
	if p.chainConfig.IsFeynman(header.Number, header.Time) && isBreatheBlock(parent.Time, header.Time) {
		...
		if !p.chainConfig.IsOnFeynman(header.Number, parent.Time, header.Time) {
			if err := p.updateValidatorSetV2(state, header, cx, txs, receipts, systemTxs, usedGas, false, firehoseContext); err != nil {
				return err
			}
		}
	}
...
func (p *Parlia) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
	txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, _ []*types.Withdrawal, firehoseContext *firehose.Context) (*types.Block, []*types.Receipt, error) {
	// No block rewards in PoA, so the state remains as is and uncles are dropped
	cx := chainContext{Chain: chain, parlia: p}
	if txs == nil {
	...
	if p.chainConfig.IsOnFeynman(header.Number, parent.Time, header.Time) {
		err := p.initializeFeynmanContract(state, header, cx, &txs, &receipts, nil, &header.GasUsed, true, firehoseContext)
		if err != nil {
			log.Error("init feynman contract failed", "error", err)
		}
	}
	...
	if p.chainConfig.IsPlato(header.Number) {
		if err := p.distributeFinalityReward(chain, state, header, cx, &txs, &receipts, nil, &header.GasUsed, true, firehoseContext); err != nil {
			return nil, nil, err
		}
	}
	...
	if p.chainConfig.IsFeynman(header.Number, header.Time) && isBreatheBlock(parent.Time, header.Time) {
		// we should avoid update validators in the Feynman upgrade block
		if !p.chainConfig.IsOnFeynman(header.Number, parent.Time, header.Time) {
			if err := p.updateValidatorSetV2(state, header, cx, &txs, &receipts, nil, &header.GasUsed, true, firehoseContext); err != nil {
				return nil, nil, err
			}
		}
	}
...
func (p *Parlia) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	state.SetBalance(consensus.SystemAddress, common.U2560, firehoseContext, firehose.BalanceChangeReason("reward_transfaction_fee"))
	...
	if doDistributeSysReward {
		...
		if rewards.Cmp(common.U2560) > 0 {
			err := p.distributeToSystem(rewards.ToBig(), state, header, chain, txs, receipts, receivedTxs, usedGas, mining, firehoseContext)
			if err != nil {
				return err
			}
	...
	return p.distributeToValidator(balance.ToBig(), val, state, header, chain, txs, receipts, receivedTxs, usedGas, mining, firehoseContext)
}
...
func (p *Parlia) applyTransaction(
	...
	if firehoseContext.Enabled() {
		firehoseContext.StartTransaction(expectedTx, uint(state.TxIndex()), header.BaseFee)

...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
	...
	return doCallWithState(ctx, b, args, header, state, timeout, globalGasCap, blockOverrides, firehoseContext)
}

func doCallWithState(ctx context.Context, b Backend, args TransactionArgs, header *types.Header, state *state.StateDB, timeout time.Duration, globalGasCap uint64, blockOverrides *BlockOverrides, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
...
func (s *BlockChainAPI) Execute(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Bytes, error) {
	...
	result, err := doCall(ctx, s.b, args, state, header, overrides, nil, 5*time.Second, s.b.RPCGasCap(), firehoseContext)

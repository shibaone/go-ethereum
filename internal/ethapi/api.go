...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, state *state.StateDB, overrides *StateOverride, timeout time.Duration, globalGasCap uint64, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
	...
	return doCallWithState(ctx, b, args, header, state, timeout, globalGasCap, firehoseContext)
}

func doCallWithState(ctx context.Context, b Backend, args TransactionArgs, header *types.Header, state *state.StateDB, timeout time.Duration, globalGasCap uint64, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
	...
	if firehoseContext.Enabled() {
		...
		receipt.Logs = state.GetLogs(dmUnsetTrxHash, header.Number.Uint64(), blockHash)
...
func (s *BlockChainAPI) CallWithState(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, state *state.StateDB, overrides *StateOverride) (hexutil.Bytes, error) {
	result, err := DoCall(ctx, s.b, args, blockNrOrHash, state, overrides, s.b.RPCEVMTimeout(), s.b.RPCGasCap(), firehose.NoOpContext)
	if err != nil {
		return nil, err
	}
...
func (s *BlockChainAPI) Execute(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Bytes, error) {
	...
	result, err := DoCall(ctx, s.b, args, blockNrOrHash, nil, overrides, 5*time.Second, s.b.RPCGasCap(), firehoseContext)

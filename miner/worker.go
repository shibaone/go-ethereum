...
func (w *worker) commitTransaction(env *environment, tx *types.Transaction, interruptCtx context.Context) ([]*types.Log, error) {
	...
	receipt, err := core.ApplyTransaction(w.chainConfig, w.chain, &env.coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, *w.chain.GetVMConfig(), interruptCtx, firehose.NoOpContext)
	if err != nil {
...
func (w *worker) generateWork(ctx context.Context, params *generateParams) (*types.Block, *big.Int, error) {
	...
	block, err := w.engine.FinalizeAndAssemble(ctx, w.chain, work.header, work.state, work.txs, work.unclelist(), work.receipts, params.withdrawals, firehose.NoOpContext)
	if err != nil {
		return nil, nil, err
	}
...
func (w *worker) commit(ctx context.Context, env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		block, err := w.engine.FinalizeAndAssemble(ctx, w.chain, env.header, env.state, env.txs, env.unclelist(), env.receipts, nil, firehose.NoOpContext)
